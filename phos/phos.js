...
// 2020-05-12 tentatively disabled
// kjs = new JSEncrypt();

export default class Phos {

    static S=window.S;

    // function 
    fgl_2je() // pack top 2 items on stack into JSON
    {
        S.push( JSON.stringify( [ S.pop(), S.pop() ] ) );
    }

    F() // variable list of parameters, single letter function name
    {
      var e, $i;
...
function fgl_cn() // length, PHP count()
{
    S.push( S.pop().length );
}
...
{
    var k=S.pop();
    var P=S[ S.length - 1 ]; // do not pop object
    S.push( P[k] );
}
...
export function F() // variable list of parameters, single letter function name
{
  //    global $S;
  var e, $i;
...
{
    // global $S;
    var $S=window.S; // call by reference
    var e, $s;
...
function FGL($a) // 5gl to php bootstrap code, use function argument as input string; add colon definition;
{
    ...
    var $vk = $xk;  
    var $sa, $sb, $bx, $S, $fn, $xs, $si, $sc, $se, $s, $WA, $va, $l, $w0;  
    var $Z=$xl;
